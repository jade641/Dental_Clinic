...
namespace Dental_Clinic.Services
{
    public class EmailService
  {
     ...
     private readonly string _fromEmail = "your-clinic-email@gmail.com";
   private readonly string _fromPassword = "your-16-char-app-password";
        private readonly string _fromName = "Jade Dental Clinic";
...
public async Task<bool> SendPasswordResetEmailAsync(string toEmail, string resetToken)
   ...
   try
     {
// Create reset link - UPDATE THIS URL FOR YOUR APP
...
<head>
 ...
 .header {{ background: linear-gradient(135deg, #0066FF 0%, #0052D9 100%); color: white; padding: 30px; text-align: center; border-radius: 8px 8px 0 0; }}
  .content {{ background: #f8f9fa; padding: 30px; }}
        .button {{ display: inline-block; background: #0066FF; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0; }}
...
{
  Console.WriteLine($"Error generating reset token: {ex.Message}");
         return null;
    }
        }
